# MASTER INDEX — Property Management Workflow (Version 2 — Full)

**Version:** V2 (Full)  
**Last updated:** 2026-02-24  
**Owner:** Property Partner (Strathmore Property Limited)  
**Audience:** Property Manager, Property Management Assistant / VA, Operations Support

---

## 1) What this documentation set covers
This library documents your **end-to-end** NZ property management workflow:

- **Landlord + property onboarding** (new management, takeover, setup)
- **Leasing** (Trade Me → TPS BookMe → applications → checks → approval → agreement)
- **Tenancy setup** (agreement + bond via TPS → Property Tree tenancy record + property file)
- **Compliance (Healthy Homes + related)** (pre-tenancy readiness + ongoing tracking + evidence storage)
- **Tenant onboarding** (welcome pack + ingoing condition report)
- **Inspections** (quarterly routine + ingoing/outgoing, confirmations, follow-ups, escalation)
- **Maintenance** (Tapi/Tarpy: triage, approvals, work orders, invoices, closeout)
- **Renewals + rent reviews** (Valua evidence + TPS renewals/extensions)
- **End of tenancy + lease breaks** (vacate, outgoing, bond, relisting, cost recovery)
- **Notices + comms** (rent increases, breaches, admin notices, recordkeeping)
- **Email management** (VA-owned loop closure; filing into Property Tree)
- **Daily/weekly/monthly QA** (nothing falls through)

**Non‑negotiable:** anything that matters must end up in the **Property Tree property record**.

---

## 2) How to use the library
- **Playbooks (01_)**: lifecycle “what happens + when” (day-to-day process).
- **SOPs (02_)**: exact “how to do it” in each system (click-by-click and field checklists).
- **Templates (03_)**: copy/paste messages + website copy (lease break guide included).
- **QA (04_)**: daily/weekly/monthly controls and hygiene.

**Golden rule:** if it isn’t recorded in Property Tree (or linked into the property record), it didn’t happen.

---

## 3) Systems map (quick)
See: **00.2_SYSTEMS_MAP_DATA_FLOW_V2.md**

### TPS / Tenancy.co.nz (leasing + agreements + renewals + compliance + apps)
- BookMe viewings (enquiry routing via Trade Me listing ID)
- Tenant portal accounts, applications, checks
- Agreement generation and digital signing
- Renewals/extensions
- Bond workflow integration (you still collect into trust first)

### Trade Me for Rent (marketing channel)
- Listing published on Trade Me
- Trade Me listing number added in TPS to route enquiries → BookMe

### Property Tree (system of record + trust accounting + property file)
- Property/owner/tenant master data, ledgers, tenancy dates
- Inspection scheduling (sync to Inspection Express)
- Weekly disbursements (Monday night run)
- **Document repository** (agreements, compliance evidence, insurance, inspection reports, notices)

### Tapi (maintenance CRM) — “Tarpy” internal naming
- Website maintenance form → Tapi jobs
- Approvals, quote/work order workflow, comms to tenant/owner/contractor
- Invoices to PropertyPartner@tarpy.co.nz; match by work order number; approve/code; sync to Property Tree

### Inspection Express (inspection execution + reporting)
- Receives inspection schedule from Property Tree
- Reports published to tenant/landlord
- Inspection-captured maintenance syncs into Tapi

### Valua (rent evidence + recommendations)
- Market evidence set and recommended rent range for owners

### Email (hello@ + owner mailbox)
- Exceptions + escalations + “edge cases”
- VA owns loop closure and filing key comms into Property Tree

---

## 4) “Most used” routes (do this, not memory)

### A) Onboard landlord + property
1) Playbook: **01_PLAYBOOK_LANDLORD_PROPERTY_ONBOARDING_V2.md**  
2) SOP: **02_SOP_PROPERTYTREE_CREATE_OWNER_PROPERTY_V2.md**  
3) SOP: **02_SOP_TPS_PROPERTY_SETUP_TRADEME_ID_BOOKME_V2.md**  
4) Playbook: **01_PLAYBOOK_COMPLIANCE_HEALTHY_HOMES_V2.md** (if gaps)  
5) Checklist: **03_TEMPLATES_CHECKLISTS_V2.md**

### B) Lease a property (end-to-end)
1) Playbook: **01_PLAYBOOK_LEASING_TRADEME_TPS_BOOKME_V2.md**  
2) SOPs: TPS BookMe + TPS applications + TPS agreement generation  
3) Handover: **01_PLAYBOOK_TENANCY_SETUP_TPS_TO_PROPERTYTREE_V2.md**

### C) Maintenance
1) Playbook: **01_PLAYBOOK_MAINTENANCE_TAPI_TARPY_V2.md**  
2) SOP: **02_SOP_TAPI_INTAKE_TRIAGE_APPROVALS_WORKORDERS_V2.md**  
3) SOP: **02_SOP_TAPI_INVOICES_OWNER_TENANT_DIY_SYNC_TO_PROPERTYTREE_V2.md**

### D) Inspections
1) Playbook: **01_PLAYBOOK_INSPECTIONS_PROPERTYTREE_TO_INSPECTIONEXPRESS_V2.md**  
2) SOP: **02_SOP_PROPERTYTREE_INSPECTION_SCHEDULING_CONFIRMATION_V2.md**  
3) SOP: **02_SOP_INSPECTION_EXPRESS_REPORT_PUBLISH_ACTIONS_TO_TAPI_V2.md**

### E) Renewals + rent reviews
1) Playbook: **01_PLAYBOOK_RENEWALS_RENT_REVIEWS_VALUA_TPS_V2.md**  
2) SOP: **02_SOP_VALUA_RENT_RESEARCH_V2.md**  
3) SOP: **02_SOP_TPS_AGREEMENT_GENERATION_SIGNING_RENEWALS_V2.md**

### F) End of tenancy / lease break
1) Playbook: **01_PLAYBOOK_END_OF_TENANCY_BREAK_LEASE_V2.md**  
2) Template: **03_TEMPLATES_NOTICES_EMAILS_V2.md** (lease break guide + emails)  
3) File everything into Property Tree

---

## 5) Full library map

### START HERE
- 00.2_SYSTEMS_MAP_DATA_FLOW_V2.md
- 00.3_STANDARDS_SLAS_APPROVALS_RECORDS_V2.md

### PLAYBOOKS (Lifecycle)
- 01_PLAYBOOK_LANDLORD_PROPERTY_ONBOARDING_V2.md
- 01_PLAYBOOK_LEASING_TRADEME_TPS_BOOKME_V2.md
- 01_PLAYBOOK_TENANCY_SETUP_TPS_TO_PROPERTYTREE_V2.md
- 01_PLAYBOOK_COMPLIANCE_HEALTHY_HOMES_V2.md
- 01_PLAYBOOK_TENANT_ONBOARDING_V2.md
- 01_PLAYBOOK_INSPECTIONS_PROPERTYTREE_TO_INSPECTIONEXPRESS_V2.md
- 01_PLAYBOOK_MAINTENANCE_TAPI_TARPY_V2.md
- 01_PLAYBOOK_RENEWALS_RENT_REVIEWS_VALUA_TPS_V2.md
- 01_PLAYBOOK_END_OF_TENANCY_BREAK_LEASE_V2.md
- 01_PLAYBOOK_NOTICES_AND_COMMS_V2.md
- 01_PLAYBOOK_EMAIL_AND_RECORDKEEPING_V2.md

### SOPs (How-To)
- 02_SOP_PROPERTYTREE_CREATE_OWNER_PROPERTY_V2.md
- 02_SOP_PROPERTYTREE_CREATE_TENANCY_DATES_V2.md
- 02_SOP_PROPERTYTREE_INSPECTION_SCHEDULING_CONFIRMATION_V2.md
- 02_SOP_TPS_PROPERTY_SETUP_TRADEME_ID_BOOKME_V2.md
- 02_SOP_TPS_VIEWINGS_BOOKME_V2.md
- 02_SOP_TPS_APPLICATION_PROCESSING_CHECKS_APPROVAL_V2.md
- 02_SOP_TPS_AGREEMENT_GENERATION_SIGNING_RENEWALS_V2.md
- 02_SOP_TENANCY_SERVICES_BOND_LODGEMENT_VIA_TPS_V2.md
- 02_SOP_COMPLIANCE_HEALTHY_HOMES_EVIDENCE_V2.md
- 02_SOP_TAPI_INTAKE_TRIAGE_APPROVALS_WORKORDERS_V2.md
- 02_SOP_TAPI_INVOICES_OWNER_TENANT_DIY_SYNC_TO_PROPERTYTREE_V2.md
- 02_SOP_INSPECTION_EXPRESS_REPORT_PUBLISH_ACTIONS_TO_TAPI_V2.md
- 02_SOP_VALUA_RENT_RESEARCH_V2.md
- 02_SOP_EMAIL_MANAGEMENT_SHARED_INBOX_V2.md
- 02_SOP_PROPERTYTREE_FILING_AND_NAMING_V2.md

### TEMPLATES + QA
- 03_TEMPLATES_NOTICES_EMAILS_V2.md
- 03_TEMPLATES_CHECKLISTS_V2.md
- 04_QA_DAILY_TRIAGE_CHECKLIST_V2.md
- 04_QA_WEEKLY_OPERATIONS_CHECKLIST_V2.md
- 04_QA_MONTHLY_COMPLIANCE_AUDIT_V2.md
- 99_SOURCES_NOTE_V2.md

---

## 6) Change control
- Update the **Playbook** first when the process changes.
- Update the **SOP** second (system steps).
- Update **Templates** last (wording).
- Log changes at the top of the doc (version + last updated).
