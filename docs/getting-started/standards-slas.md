...
- Acceptance timeline: expect contractor to accept work order within 1–2 days (Tapi auto-sends reminders every 12–24h). If not accepted in 2 days, send manual follow-up.
...
- For urgent/same-day jobs: **phone the contractor first** to confirm availability, then send Tapi work order as formal record.
...
### Default
Most maintenance requires owner approval before actioning. Use the Tapi "Ask owner for approval" action with a personalised message.

### Triage decision framework (from video training)

!!! tip "Quick Reference — Approval Thresholds"

    - **< $200–300** (obvious scope): no approval needed — send work order directly
    - **$300–500** (clear scope): owner approval required
    - **> $500** (or uncertain scope): request quotes first, then present to owner

| Condition | Action | Approval needed? |
|---|---|---|
| Active leak, flooding, no hot water, power outage, sewerage, serious electrical hazard | Send work order immediately. Phone contractor first if same-day needed. | No — act first, notify owner simultaneously |
| Safety/compliance issue (fire doors, smoke alarms) | Send work order + inform owner | No — courtesy notification only |
| Priority chattels broken (oven, cooktop, dishwasher, rangehood, heat pump) | Fast-track approval request or direct work order if owner relationship allows | Preferred but not blocking |
| Simple/cheap fix (<$200–300, obvious scope, e.g., broken curtain bracket, basic light replacement) | Send work order directly | No — too minor to bother owner |
| Moderate routine item ($300–500, clear scope) | Ask owner for approval in Tapi, then work order once approved | Yes |
| Expensive or uncertain scope (>$500 or diagnostic needed) | Request quotes first, then present to owner | Yes — quote-first |
| Owner states they'll handle it | Send to owner for DIY in Tapi. Notify all tenants. | N/A |
| Non-urgent inspection recommendation | Ask owner for approval. Many will not respond. Follow up Day 3/7. | Yes — but acceptable if owner doesn't act |
...
### Quote-first guide
If likely > $500, large scope, or uncertainty about the work required:
- Request quote first (unless urgent).
- Use the "knowable scope" test: can the contractor estimate without visiting? (e.g., spouting clean = yes, plumbing leak = no)
- PM may give the owner a verbal price range first; owner may still want a formal quote.
